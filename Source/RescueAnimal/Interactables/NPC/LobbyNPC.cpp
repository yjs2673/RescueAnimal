...
ENPCDialogueState ALobbyNPC::GetCurrentDialogueState() const
{
	...
	const URAGameInstance* RAGameInstance = World
		? Cast<URAGameInstance>(World->GetGameInstance())
		: nullptr;
	...
	if (!RAGameInstance->HasPlayedIntroDialogue())
	{
		return ENPCDialogueState::Intro;
	}

	if (RAGameInstance->AreAllFieldMapsCleared() && !RAGameInstance->IsEndingTriggered())
	{
...
void ALobbyNPC::Interact()
{
	...
	URAGameInstance* RAGameInstance = Cast<URAGameInstance>(
		UGameplayStatics::GetGameInstance(this)
	);

	if (!RAGameInstance)
	{
		UE_LOG(LogTemp, Warning, TEXT("[LobbyNPC] Interaction skipped: RAGameInstance is unavailable."));
		return;
	}
...
void ALobbyNPC::StartEndingDialogue()
{
	...
	if (!RAGameInstance)
	{
		UE_LOG(LogTemp, Warning, TEXT("[LobbyNPC] Ending dialogue skipped: RAGameInstance is unavailable."));
		return;
	}

	if (RAGameInstance->IsEndingTriggered())
	{
		UE_LOG(LogTemp, Log, TEXT("[LobbyNPC] Ending dialogue skipped: Ending was already triggered."));
		return;
	}

	if (!RAGameInstance->AreAllFieldMapsCleared())
	{
...
void ALobbyNPC::OnEndingDialogueFinished()
{
	...
	if (!RAGameInstance)
	{
		UE_LOG(LogTemp, Warning, TEXT("[LobbyNPC] Ending was not completed: RAGameInstance is unavailable."));
		EnablePlayerControlAfterDialogue();
		return;
	}
	...
	RAGameInstance->SetEndingTriggered(true);
...
{
	const URAGameInstance* RAGameInstance = Cast<URAGameInstance>(
		UGameplayStatics::GetGameInstance(this)
	);
	...
	const FString PlainStatus = RAGameInstance->IsMapCleared(TEXT("MAP_Plain"))
		? TEXT("클리어 완료")
		: TEXT("진행 중");
	const FString SnowStatus = RAGameInstance->IsMapCleared(TEXT("MAP_Snow"))
		? TEXT("클리어 완료")
		: TEXT("진행 중");
	const FString DesertStatus = RAGameInstance->IsMapCleared(TEXT("MAP_Desert"))
		? TEXT("클리어 완료")
		: TEXT("진행 중");
...
void ALobbyNPC::TryAutoStartIntroDialogue()
{
	...
	if (!RAGameInstance)
	{
		UE_LOG(LogTemp, Warning, TEXT("[LobbyNPC] Auto intro skipped: RAGameInstance is unavailable."));
		return;
	}
...
void ALobbyNPC::OnIntroDialogueFinished()
{
	...
	if (RAGameInstance)
	{
		RAGameInstance->SetHasPlayedIntroDialogue(true);
		RAGameInstance->SetGameStarted(true);
	}
	else
	{
		UE_LOG(LogTemp, Warning, TEXT("[LobbyNPC] Intro state was not saved: RAGameInstance is unavailable."));
	}
...
void ALobbyNPC::HandleDialogueChoiceSelected(EDialogueChoice SelectedChoice)
{
	...
	default:
		...
		if (const URAGameInstance* RAGameInstance = Cast<URAGameInstance>(
			UGameplayStatics::GetGameInstance(this)))
		{
			if (RAGameInstance->AreAllFieldMapsCleared() && !RAGameInstance->IsEndingTriggered())
			{
			...
			else if (!RAGameInstance->AreAllFieldMapsCleared())
			{
				ShowEndingLockedDialogue();
			}
			else
		...
		{
			UE_LOG(LogTemp, Warning, TEXT("[LobbyNPC] Ending choice failed: RAGameInstance is unavailable."));
		}
		break;
	}
}
