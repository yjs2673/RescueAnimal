	...
	const FHitResult& SweepResult)
{
	...
	{
		const bool bOwnedByPlayer = OwnerActor && OwnerActor->IsA<ARACharacter>();
		if (bOwnedByPlayer && !OtherActor->IsA<ARAEnemyBase>())
		{
			UE_LOG(LogTemp, Warning, TEXT("Player arrow ignored non-enemy actor: %s"), *OtherActor->GetName());
			return;
		}

		const bool bOwnedByEnemy = OwnerActor && OwnerActor->IsA<ARAEnemyBase>();
		if (bOwnedByEnemy && OtherActor->IsA<ARAEnemyBase>())
		{
		...
		if (const ARACharacter* PlayerCharacter = Cast<ARACharacter>(OtherActor))
		{
